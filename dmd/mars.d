    ...
    for (size_t i = 1; i < arguments.dim; i++)
    {
        ...
        else if (arg == "-run")              // https://dlang.org/dmd.html#switch-run
        {
            ...
            if (length)
            {
                ...
                if (ext && FileName.equals(ext, "d") == 0 && FileName.equals(ext, "dex") == 0 && FileName.equals(ext, "di") == 0)
                {
                    error("-run MUST be followed by a source file, not '%s' '%s'", arguments[i + 1],ext);
                    break;
                }
                if (runarg == "-")
...
version (IN_LLVM) {} else
{
    ...
    /* Examine extension to see if it is a valid
        ...
        */
    if (FileName.equals(ext, global.mars_ext) || FileName.equals(ext, "dex") || FileName.equals(ext, global.hdr_ext) || FileName.equals(ext, "dd"))
    {
